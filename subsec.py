...
def get_local_stats(lines):
    ...
    This serves as the 'baseline' to detect headers.
    """
    if not lines:
        return 10.0, "Unknown"
...
def extract_subsections(section_lines):
    ...
    Records which line index is a subsection header and why (scoring breakdown).
    Returns a list of dictionaries: 
    [ {'title': 'Software Engineer', 'line_index': 5, 'score': 5, 'score_breakdown': {...}, 'content': [...]}, ... ]
    """
    if not section_lines:
        return []

    # 1. Get baseline for this specific section
    ...
    for idx, line in enumerate(section_lines):
        # Get previous line for gap analysis
        prev_line = section_lines[idx-1] if idx > 0 else None
        
        is_header, total_score, breakdown = is_subsection_header(line, prev_line, local_stats, section_lines, idx)
        ...
        if is_header:
            ...
            # Start new subsection with scoring details
            current_sub = {
                "title": line["text"],
                "line_index": idx,  # Track which line is the subsection header
                "score": total_score,  # Total score
                "score_breakdown": breakdown,  # Which rules triggered
                "content": []
            }
        else:
            if current_sub:
...
def main():
    ...
    # 3. Build output structure with subsection line indices
    output_data = {}
    ...
    # 4. Save to subsec.json
    with open("subsec.json", "w") as f:
        json.dump(output_data, f, indent=4, default=str)
    
    print(f"Results saved to subsec.json")
